...
class ProjectDataManager:
    ...
    def browse_sqlite_path(self):
        path = filedialog.askopenfilename(title="Select SQLite Database File")
        if path:
            self.sqlite_path_var.set(path)

    def browse_venv_path(self):
        directory = filedialog.askdirectory(title="Select Virtual Environment Directory")
        if directory:
            self.venv_path_var.set(directory)
