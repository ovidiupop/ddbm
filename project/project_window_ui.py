...
def create_project_section(parent, data_manager):
    ...
    ttk.Checkbutton(frame, text="Use Virtual Environment", variable=data_manager.use_venv_var,
                    command=lambda: toggle_venv(data_manager, lambda: adjust_window_size(parent.master, parent))).grid(row=2, column=0, columnspan=3, sticky="w", pady=(5, 0))
    ...
    toggle_venv(data_manager, lambda: adjust_window_size(parent.master, parent))

def create_db_sections(parent, data_manager, available_dbs):
    db_configs = [
    ...
    for config in db_configs:
        if config[0].lower() in [db.lower() for db in available_dbs]:
            create_db_section(parent, data_manager, *config)

def create_db_section(parent, data_manager, title, var, path_var=None, browse_command=None):
    frame = ttk.LabelFrame(parent, text=f"{title} Backup", padding="10")
    frame.pack(fill=tk.X, pady=(0, 10))
    frame.columnconfigure(1, weight=1)

    ttk.Checkbutton(frame, text=f"Enable {title} Backup", variable=var,
                    command=lambda: toggle_db_path(title, frame, data_manager, lambda: adjust_window_size(parent.master, parent))).grid(row=0, column=0, columnspan=3, sticky="w")
    ...
    if path_var and browse_command:
        ...
        toggle_db_path(title, frame, data_manager, lambda: adjust_window_size(parent.master, parent))
